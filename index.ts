...
bot.onText(
  ...
  async (msg: Message, match: RegExpExecArray | null) => {
    ...
    if (!match || match.length < 3 || !match[1] || !match[2]) {
      bot.sendMessage(
        chatId,
        "Please provide both chain and token address. Example: /map bsc 0x603c7f932ed1fc6575303d8fb018fdcbb0f39a95"
      );
      return;
    }
...
bot.onText(
  ...
  async (msg: Message, match: RegExpExecArray | null) => {
    ...
    if (!match || match.length < 3) {
      bot.sendMessage(
        chatId,
        "Please provide both chain and token address. Example: /score bsc 0x603c7f932ed1fc6575303d8fb018fdcbb0f39a95"
      );
      return;
    }
    ...
    bot.sendMessage(
      chatId,
      `🔍 Fetching decentralization score for ${token} on ${chain}...`
    );
    ...
    try {
      ...
      let scoreEmoji = "🟢";
      if (
        metadata.decentralisation_score &&
        metadata.decentralisation_score < 50
      ) {
        scoreEmoji = "🔴";
      } else if (
        metadata.decentralisation_score &&
        metadata.decentralisation_score < 70
      ) {
        scoreEmoji = "🟠";
      }

      const message = `${scoreEmoji} *Decentralization Score for ${
        mapData.full_name
      } (${mapData.symbol})*
...
*Supply in Contracts:* ${
        metadata.identified_supply?.percent_in_contracts || "N/A"
      }%
...
bot.onText(
  ...
  async (msg: Message, match: RegExpExecArray | null) => {
    ...
    if (!match || match.length < 3) {
      bot.sendMessage(
        chatId,
        "Please provide both chain and token address. Example: /screenshot bsc 0x603c7f932ed1fc6575303d8fb018fdcbb0f39a95"
      );
      return;
    }
    ...
    bot.sendMessage(
      chatId,
      `📸 Generating screenshot for ${token} on ${chain}. This may take a moment...`
    );

    // Start the screenshot generation in the background
    // This allows the serverless function to return quickly
    generateMapScreenshot(chain, token)
      .then(async (screenshot) => {
        if (!screenshot) {
          bot.sendMessage(
            chatId,
            "❌ Failed to generate screenshot. The map might not be available."
          );
          return;
        }
...
bot.onText(
  ...
  async (msg: Message, match: RegExpExecArray | null) => {
    ...
    if (!match || match.length < 3) {
      bot.sendMessage(
        chatId,
        "Please provide both chain and token address. Example: /holders bsc 0x603c7f932ed1fc6575303d8fb018fdcbb0f39a95"
      );
      return;
    }
    ...
    try {
      ...
      topHolders.forEach((holder: MapNode, index: number) => {
        ...
        holdersText += `${index + 1}. ${name.substring(0, 20)}${
          name.length > 20 ? "..." : ""
        }\n`;
...
bot.on("message", (msg: Message) => {
  ...
  if (!msg.text?.startsWith("/")) {
    bot.sendMessage(
      chatId,
      `🤖 I received: "${msg.text}". Type /help for options.`
    );
  }
});
